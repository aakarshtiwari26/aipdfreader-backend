...
router.post("/upload", upload.single("pdf"), async (req, res) => {
  try {
    ...
    const dataBuffer = req.file.buffer;
    const data = await pdfParse(dataBuffer, { max: 10 });
    const text = data.text;
    const pageCount = data.numpages;
    ...
    if (!text || text.length < 100) {
      return res.status(400).json({ error: "PDF text too short or unreadable" });
    }
    ...
    const summaryResponse = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: text.substring(0, 800) },
      ],
      max_tokens: 75,
      temperature: 0.5,
    });
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Context: ${context.substring(0, 800)}\nQuestion: ${question}`,
        },
      ],
      max_tokens: 100,
      temperature: 0.5,
